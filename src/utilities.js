...
function compareIngredientsCB(ingredientA, ingredientB){
    ...
    if (ingredientB.aisle > ingredientA.aisle) return -1;
    if (ingredientB.aisle < ingredientA.aisle) return 1;
    if (ingredientB.name > ingredientA.name) return -1;
    return 1;
...
function dishType(dish){
    // TODO
    if (!dish.dishTypes?.find(isKnownTypeCB)) return "";
...
function menuPrice(dishesArray){
    ...
    const arrayOfPrices = dishesArray.map(priceCB);
    ...
    const result = arrayOfPrices.reduce(addCB, 0);
    return result;
}
